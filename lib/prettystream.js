...
function PrettyStream(opts){
  ...
  function extractTime(rec){
    ...
    var short = config.mode === 'short' || config.mode === 'dev'

    if (config.locale) {
      time = new Date(time);
      time = !short ? time.toLocaleString(config.locale) : time.toLocaleTimeString(config.locale);
      return stylize(time);
    } else if (short && time[10] == 'T') {
  ...
  function extractCustomDetails(rec){
    ...
    Object.keys(rec).forEach(function(key) {
      var err = config.error && key === 'err' && !rec[key].stack;
      if (err || skip.indexOf(key) === -1){
        var value = rec[key];
...
module.exports = function(opts, pipe) {
  var stream = new PrettyStream(opts);
